## Hi there 👋 I'm Jeptum Brenda

- Software Engineer | TechMommaHive Founder | Women in Tech Advocate | Tech for Good Advocate
- Building sleek, user-friendly apps that solve real-world problems—especially in **finance** and **career development**.
- Code by day, empowering moms in tech by night. Let’s create, innovate, and break barriers together!

## What I’m up to:
- Currently working on: TechMommaHive—a platform that provides Career Development & Coaching for Moms.
- Currently learning: Cloud Engineering (Diploma at AltSchool Africa), Kubernetes, and advanced DevOps practices.
- Looking to collaborate on: FinTech, EdTech, HR Tech, AI-powered tools, and impactful backend/cloud projects.
- Looking for help with: Scaling TechMommaHive and integrating AI-powered coaching features.
- Ask me about: Backend development (Python/Django, Node.js/Express), DevOps (Docker, CI/CD), tech career transitions, or balancing tech + family.


## Tech Stack
Python (Flask, Django) • JavaScript (Node.js, React, Express) • Docker • Kubernetes (learning) • MongoDB • PostgreSQL • MySQL • Git • CI/CD (GitHub Actions) • API Security • Bash

## More About Me
📸 **Lover of clean code and efficient systems** | 🏆 **Problem-solver at heart**
🎮 **Techie by day, storyteller & lifelong learner by night**
⚡ **Fun fact:** Switched from insurance underwriting to software engineering—best decision ever!
🚀 **Actively seeking Software engineer roles (Backend or Cloud-focused!** where I can build impactful solutions and grow my skills


## 📫 Connect with Me
[Email](mailto:brendajeptum@gmail.com)  
[LinkedIn](https://www.linkedin.com/in/jeptumbrenda/)  
[GitHub](https://github.com/bjeptum)
